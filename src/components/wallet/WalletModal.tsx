...
export default function WalletModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/70 p-0 backdrop-blur-lg sm:items-center sm:p-4" onClick={(e)=>{ if(e.target===e.currentTarget) onClose(); }}>
    <div role="dialog" aria-modal="true" aria-label="Wallet connection" className="max-h-[88dvh] w-full overflow-y-auto rounded-t-3xl border border-white/10 bg-[#0d1224] p-5 text-white shadow-2xl sm:max-w-md sm:rounded-3xl sm:p-6">
      <div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-verse-blue">Polygon wallet</p><h2 className="mt-1 text-xl font-black">Connect Wallet</h2></div><button type="button" onClick={onClose} className="rounded-xl bg-white/10 p-2"><X size={18}/></button></div>
      ...
      {isConnected && address ? <div className="mt-5 space-y-3">
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-xs text-gray-400">Connected wallet</p><p className="mt-1 font-mono text-sm">{shortAddress(address, 6)}</p><p className="mt-2 text-xs text-gray-400">Network: <span className={chain?.id===137 ? "text-emerald-300" : "text-amber-300"}>{chain?.id===137 ? "Polygon" : chain?.name || "Wrong network"}</span></p></div>
        ...
        <p className="text-sm font-bold text-gray-200">Installed wallets</p>
        {connectors.map((connector) => <button key={connector.uid} type="button" disabled={isPending} onClick={() => connect({ connector, chainId: 137 })} className="flex min-h-12 w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-left font-bold hover:bg-white/[0.08]"><span>{connector.name}</span><span className="text-xs text-gray-400">Polygon</span></button>)}
        <p className="pt-2 text-xs leading-5 text-gray-400">Mobile wallet / QR is available when WalletConnect project ID is configured.</p>
