  ...
  return (
    <group>
      <points ref={points} geometry={geometry}>
        <pointsMaterial size={mobile ? .038 : .030} vertexColors transparent opacity={.56} sizeAttenuation depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
      </points>
      {bases.map((base, index) => (
        <mesh key={index} ref={(node) => { motes.current[index] = node; }} position={[base.x, base.y, base.z]}>
          <sphereGeometry args={[base.size, 8, 8]} />
          <meshBasicMaterial color={index % 3 === 0 ? "#c8a0f0" : "#9048d8"} transparent opacity={.58} blending={THREE.AdditiveBlending} depthWrite={false} toneMapped={false} />
        </mesh>
      ))}
    </group>
  );
