...
export default function SuccessPage({ params }: { params: { publicOrderId: string } }) {
  return <main className="min-h-screen bg-[#0B0D14] flex items-center justify-center px-4 text-white">
    <section className="mx-auto max-w-xl rounded-[2rem] border border-green-400/30 bg-green-400/10 p-8 text-center">
      ...
      <p className="mt-4 text-sm text-gray-300">Your order <code className="font-mono text-verse-blue">{params.publicOrderId}</code> has been verified on Polygon.</p>
      <p className="mt-2 text-sm text-gray-400">A receipt has been sent to your email. You can verify this payment publicly at any time.</p>
