...
export default async function StudioOrders({searchParams}:{searchParams:{status?:string;q?:string}}){
 ...
 return <StudioShell email={user.email||"owner"}><h1 className="font-sora text-3xl font-black">Orders</h1><form className="mt-5 grid gap-2 rounded-2xl bg-white/[0.04] p-3 sm:grid-cols-[1fr_auto_auto]"><input name="q" defaultValue={searchParams.q} placeholder="Search order, client, service" className="min-h-11 rounded-xl border border-white/10 bg-black/20 px-4 text-base outline-none"/><select name="status" defaultValue={searchParams.status} className="min-h-11 rounded-xl bg-black/30 px-4"><option value="">All statuses</option>{["pending","paid","reviewing","accepted","in_progress","delivered","completed","cancelled"].map(x=><option key={x}>{x}</option>)}</select><button className="rounded-xl bg-verse-purple px-5 font-black">Filter</button></form><div className="mt-5 overflow-x-auto rounded-2xl border border-white/10"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-white/5 text-xs uppercase text-gray-500"><tr><th className="p-4">Order</th><th>Client</th><th>Service</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>{(orders||[]).map(order=><tr key={order.id} className="border-t border-white/5"><td className="p-4"><Link className="font-mono font-bold text-verse-blue" href={`/studio/orders/${order.id}`}>{order.public_order_id}</Link></td><td>{order.customer_name||"—"}</td><td>{order.slug}</td><td>{order.amount_human} {order.token_symbol}</td><td><span className="rounded-full bg-white/10 px-2 py-1 text-xs">{order.status}</span></td><td>{new Date(order.created_at).toLocaleDateString()}</td></tr>)}</tbody></table>{!orders?.length&&<p className="p-8 text-center text-gray-500">No orders match this view.</p>}</div></StudioShell>;
}
